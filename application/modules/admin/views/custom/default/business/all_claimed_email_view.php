        ...
        <?php }else{?>
           ...
           <tbody>
        	...
        	<?php $i=$start+1;foreach($posts->result() as $row):  
                ...
                $query = $CI->post_model->get_claimed_post_by_id($row->post_id);
                if(!is_bool($query)){
                $post = $query->row();
          ?>
               ...
               <tr>
                  ...
                  <td data-title="<?php echo lang_key('title'); ?>" class="numeric"><?php echo get_post_data_by_lang($post,'title');?></td>
